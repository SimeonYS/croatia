...
class CroatiaPipeline:

    # Database setup
    conn = sqlite3.connect('croatia.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `croatia`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM croatia WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO croatia (date, title, link, content)"
